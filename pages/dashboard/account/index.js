 ...
 * Created Date: Tuesday, July 6th 2021, 10:31:17 am                           *
 * -----                                                                       *
 * Last Modified: Tuesday, July 6th 2021 10:31:18 am                           *
 */
...
import { kUserId } from "../../../utils/constants";
...
function AccountSettings() {
  ...
  useEffect(() => {
    firebase.auth().onAuthStateChanged((user) => {
      // url
      let id = user?.uid || localStorage.getItem(kUserId);

      let db = firebase.firestore();
      db.doc(`members/${id}`).onSnapshot(
        (snapshot) => {
          if (snapshot.exists) {
            setUser(snapshot.data());
            setEditableUser(snapshot.data());
            setLoading(false);
          }
        },
        (err) => {
                ...
                {hasPickedImage && (
                  <>
                    <button
                      ...
                      className={`${
                        uploading ? "btn-outlined" : "btn-primary"
                      }`}
                      onClick={uploadImage}
                    >
                      <h6 className="">
